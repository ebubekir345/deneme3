...
const MoreActionScreen: FC = (): ReactElement => {
  ...
  const [_, rebinAction] = useRebinStore();

  return (
    <>
      <Box
        onClick={() => rebinAction.setIsMoreActionsOpen(false)}
        position="fixed"
        width="100vw"
        height="100vh"
        left={0}
      ...
      >
        <Box width="fit-content">
          <Button
            onClick={() => {
              rebinAction.setIsMoreActionsOpen(false);
              rebinAction.setIsManuelBarcodeInputOpen(true);
            }}
            variant="alternative"
            fontFamily="Jost"
            fontWeight={500}
        ...
        <Box width="fit-content">
          <Button
            onClick={() => {
              rebinAction.setIsMoreActionsOpen(false);
              rebinAction.toggleModalState(RebinModals.Logout);
            }}
            variant="light"
            fontFamily="Jost"
            fontWeight={500}
