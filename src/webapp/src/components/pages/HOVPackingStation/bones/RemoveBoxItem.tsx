...
const RemoveBoxItem: React.FC<RemoveBoxItemProps> = ({ box, boxItemListRef, bottomButtonGroupRef }): ReactElement => {
  ...
  const removeCargoPackageResponse: Resource<any> = useSelector(
    (state: StoreState) => state.resources[ResourceType.RemoveHovCargoPackage]
  );
  const unassignQuarantineToteResponse: Resource<any> = useSelector(
    (state: StoreState) => state.resources[ResourceType.UnassignHovQuarantineTote]
  );

  useEffect(() => {
    if (removeCargoPackageResponse?.isSuccess || unassignQuarantineToteResponse?.isSuccess) {
      if (packingState.boxItemToBeRemoved) {
        const isSelectedBoxRemoved = packingState.boxItems.find(
          item => packingState.boxItemToBeRemoved?.key === item.key
        )?.selected;
        const prevBoxItems = packingState.boxItems;
        packingAction.setBoxItems(
          isSelectedBoxRemoved
            ? prevBoxItems
                .filter(item => packingState.boxItemToBeRemoved?.key !== item.key)
                .map((item, i) => {
                  return {
                    ...item,
                    selected: i === 0,
        ...
        packingState.boxItemToBeRemoved.content.map(removedItem => {
          let tempOrderItem;
        ...
        let totalPackedItemCount = packingState.trolleyDetails.packedCount;
        packingState?.boxItemToBeRemoved?.content.map(item => {
          totalPackedItemCount -= item.count;
        });
        packingAction.setTrolleyDetails({ ...packingState.trolleyDetails, packedCount: totalPackedItemCount });
  ...
  }, [removeCargoPackageResponse, unassignQuarantineToteResponse, packingState.boxItemToBeRemoved]);

  const onRemoveBoxItem = (box: BoxItemsInterface) => {
    packingAction.setItemToBeRemove(box);
    if (packingState.isMissing || packingState.isCancelled) {
      dispatch(
        resourceActions.resourceRequested(ResourceType.UnassignHovQuarantineTote, {
          payload: { packingQuarantineProcessId: packingState.processId, quarantineToteLabel: box.title },
        })
      );
    } else {
      dispatch(
        resourceActions.resourceRequested(ResourceType.RemoveHovCargoPackage, {
          payload: {
