...
const InProgressOrderPanel: React.FC<IInProgressOrderPanel> = ({ isCompletePacking, setIsCompletePacking }) => {
  ...
  const isCompletePackingBusy = useSelector((state: StoreState) =>
    resourceSelectors.isBusy(state.resources, ResourceType.CompleteHovPackingProcess)
  );
  const isCompleteQuarantineBusy = useSelector((state: StoreState) =>
    resourceSelectors.isBusy(state.resources, ResourceType.CompleteHovPackingQuarantineProcess)
  );
  ...
  useEffect(() => {
    if (isCompletePacking && isOrderCompleted) {
      ...
      return packingAction.callInfoMessageBox({
        ...
        text: t(`TouchScreenƒ.Barcode.Error`),
      });
    }
  ...
  const onCompletePacking = () => {
    ...
    dispatch(
      resourceActions.resourceRequested(ResourceType.CompleteHovPackingProcess, {
        params: {
    ...
    dispatch(
      resourceActions.resourceRequested(ResourceType.PrintUnprintedHOVPalettePackingList, {
