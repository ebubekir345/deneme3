...
const ReturnDialogModal: FC = (): ReactElement => {
  ...
  const rasPickCloseStationResponse = useSelector(
    (state: StoreState) => state.resources[ResourceType.RasPickCloseStation]
  );

  useEffect(() => {
    if (rasPickCloseStationResponse?.isSuccess) {
      sendSlotMessage({
  ...
  return (
    ...
    >
      <ActionButton
        onClick={() => pickingAction.toggleModalState(RASPickingModals.Logout)}
        height={48}
        width={172}
      ...
      <ActionButton
        onClick={() =>
          dispatch(
            resourceActions.resourceRequested(ResourceType.RasPickCloseStation, {
              payload: {
