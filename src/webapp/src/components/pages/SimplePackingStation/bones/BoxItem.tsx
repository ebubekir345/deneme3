...
const BoxItem: FC<BoxItemProps> = ({ boxItem }): ReactElement => {
  ...
  return (
    <Box
      width={packingState.isOrderCompleted ? 'calc(50% - 16px)' : 1}
      mx={packingState.isOrderCompleted ? 8 : 0}
      mb={16}
      boxShadow="small"
      height="fit-content"
      zIndex={0}
    ...
    >
      <Flex
        height={52}
        px={16}
        bg={boxItem.selected && !packingState.isOrderCompleted ? 'palette.softBlue' : 'palette.slate_lighter'}
      ...
      <Box width={1} px={26} py={8} bg="palette.white" borderBottomRightRadius="md" borderBottomLeftRadius="md">
        {boxItem.content.map((item, i) => (
          ...
          >
            <Image src={item.imageUrl} borderRadius="full" width={38} height={38} flexShrink={0} boxShadow="small" />
          ...
          <Flex color="palette.hardBlue" fontSize="16" alignItems="center" py={16}>
            <Flex flexGrow={1} fontWeight={500} letterSpacing="negativeLarge" color="palette.grey_lighter">
              {t(`${intlKey}.PackingStation.Package.Empty`, {
                type: t(`${intlKey}.PackingStation.Package.Types.Box`),
              })}
            </Flex>
          </Flex>
        )}
