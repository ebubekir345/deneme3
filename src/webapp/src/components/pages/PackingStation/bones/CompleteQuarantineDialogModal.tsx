...
function CompleteQuarantineDialogModal() {
  ...
  const createPackingQuarantineProcessResponse: Resource<CreatePackingQuarantineProcessOutputDTO> = useSelector(
    (state: StoreState) => state.resources[ResourceType.CreatePackingQuarantineProcess]
  );
  ...
  const dispatchCompleteQuarantine = () => {
    ...
    dispatch(
      resourceActions.resourceRequested(ResourceType.CompletePackingQuarantineProcess, {
        payload: {
    ...
    packingAction.toggleModalState(PackingModals.CompleteQuarantine, false);
  };

  useEffect(() => {
    if (placeQuarantineToteToQuarantineAddressResponse?.isSuccess) {
      packingAction.toggleModalState(PackingModals.QuarantineAreaScan);
  ...
  return (
    <>
      ...
      >
        ...
        <ActionButton
          onClick={() => {
            packingAction.toggleModalState(PackingModals.CompleteQuarantine);
            packingAction.toggleModalState(PackingModals.QuarantineAreaScan);
          }}
          height="48px"
          width="126px"
