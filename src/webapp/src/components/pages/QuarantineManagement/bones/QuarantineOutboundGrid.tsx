...
const intlKey = 'QuarantineManagement.OutboundGrid';
const titleKey = 'QuarantineManagement.OutboundGrid.Title';

const QuarantineOutboundGrid: React.FC = () => {
  const { t } = useTranslation();
  const QuarantineOutboundGridColumns: Array<any> = [
    {
      name: '#',
      key: 'index',
      type: 'number',
  ...
  return (
    <GenericDataGrid
      titleKey={t(titleKey)}
      intlKey={intlKey}
      gridKey={GridType.QuarantineOutbound}
      columns={QuarantineOutboundGridColumns}
      predefinedFilters={[]}
    />
  );
};

export default QuarantineOutboundGrid;
