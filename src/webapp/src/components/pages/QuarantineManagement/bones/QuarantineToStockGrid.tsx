...
const intlKey = 'QuarantineManagement.ToStockGrid';
const titleKey = 'QuarantineManagement.ToStockGrid.Title';

const QuarantineToStockGrid: React.FC = () => {
  const { t } = useTranslation();
  const QuarantineToStockGridColumns: Array<any> = [
    {
      name: '#',
      key: 'index',
      type: 'number',
  ...
  return (
    <GenericDataGrid
      titleKey={t(titleKey)}
      intlKey={intlKey}
      gridKey={GridType.QuarantineToStock}
      columns={QuarantineToStockGridColumns}
      predefinedFilters={[]}
    />
  );
};

export default QuarantineToStockGrid;
